...
const resolveConnections = function(
...
const resolveDisconnections = function(attachments: Attachment[]) {
  ...
  do {
    ...
    for (let i = 1; i < attachments.length - 1; i++) {
      ...
      if (
        (b.side == Side.left && angle > Math.PI * 1.8) ||
      ...
      cables.forEach(cable => {
        ...
        for (let i = 0; i < attachments.length - 1; i++) {
          ...
          for (let j = 0; j < attachments.length - 1; j++) {
            ...
            if (
              lineLineIntersect(a1.outPos!, b1.inPos!, a2.outPos!, b2.inPos!)
            ) {
        ...
        if (
          hasPower &&
          finishEntity.finish.connected &&
          !cable.cable.overpowered &&
          numPoweredSpools === numSpools
        ) {
          onLevelCompleted()
        }
