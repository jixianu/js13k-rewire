...
const v01 = { x: 0, y: 1 }
const v11 = { x: 1, y: 1 }
const n21 = (v: Vec2): number =>
  ((Math.sin(v.x * 100 + v.y * 6574) + 1) * 564) % 1
...
const createCoilSprite = (size: number): Canvas => {
  ...
  const image = generateImage(
    ...
    v => {
      ...
      let grid =
        hexFieldBrightness *
        smoothstep(0.3, 1, 1 - createHexField(v, hexFieldScale)) // TODO: FOR SPOOL
      const gridShadow =
        1 -
        (smoothstep(1 - ringWidth * 0.65, 1 - ringWidth - gridShadowBlur, d) -
      ...
      const ringMask =
        smoothstep(1 - ringWidth, 1 - ringWidth + sw, d) +
        smoothstep(buttonSize, buttonSize - sw, d)
      const spriteCol = mixCol(
        mulCol(gridColor, grid),
        mulCol(metalColor, metalPlate),
        ringMask
      )
...
const createIsolatorSprite = (size: number): Canvas => {
  ...
  const gridColor = newCol(0.815, 0.2705, 0.2, 1) // isolate red
  const metalColor = newCol(1, 1, 1, 1)
  const shadowBlur = 0.2
  const shadowDistance = 0.04
  const shadowScale = 1.1
  ...
  const image = generateImage(
    ...
    v => {
      ...
      let grid =
        hexFieldBrightness *
        smoothstep(0.02, 0.41, 1 - createHexField(v, hexFieldScale)) // TODO FOR ISOLATOR
      const gridShadow =
        1 -
        (smoothstep(1 - ringWidth * 0.65, 1 - ringWidth - gridShadowBlur, d) -
          smoothstep(buttonSize + gridShadowBlur, buttonSize * 0.85, d))
...
// 创建放射关
const createGlow = (color: Color): Canvas =>
...
// 创建环光
const createRingGlow = (color: Color): Canvas =>
  generateImage(62, 62, v => {
    const cv = subV(v, halfV)
    const d = 1 - lenV(cv) * 2
    const result = mixCol(
      color,
      white,
      smoothstep(0.45, 0.5, d) * smoothstep(0.55, 0.5, d)
    )
...
const createConnectorButtons = (lightColor: Color, size: number): Canvas => {
  ...
  const image = generateImage(size, size, v => {
    ...
    // back
    const backAlpha = smoothstep(1, 0.96, py)
    let shading = smoothstep(0.9, 0.8, py) * 0.3 + 0.3
    shading -= smoothstep(0.7, 0.6, py) * smoothstep(0.2, 0.3, py) * 0.4
...
const createGameBackground = (): Canvas => {
  ...
  const highlight = generateImage(128 * 2, 72 * 2, v => {
    const w = 0.01
    const c =
      smoothstep(0, w * 0.6, v.x) *
      smoothstep(1, 1 - w * 0.6, v.x) *
      smoothstep(0, w, v.y) *
      smoothstep(1, 1 - w, v.y)
