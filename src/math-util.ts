...
const lineLineIntersect = (
  ...
  const s =
    (-s1_y * (line1a.x - line2a.x) + s1_x * (line1a.y - line2a.y)) /
    (-s2_x * s1_y + s1_x * s2_y)
  const t =
    (s2_x * (line1a.y - line2a.y) - s2_y * (line1a.x - line2a.x)) /
    (-s2_x * s1_y + s1_x * s2_y)
...
const lineCircleIntersect = (
  ...
  if (u >= 0 && u <= 1) {
    dist =
      (lineA.x + v1x * u - circle.x) ** 2 + (lineA.y + v1y * u - circle.y) ** 2
  } else {
...
// 判断点在线的左右
const sideOfLine = (p1: Vec2, p2: Vec2, p: Vec2): Side =>
